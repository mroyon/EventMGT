...
namespace AppConfig.EncryptionHandler
{
    public class EncryptionHelper
    {
        ...
        public List<string> GetPasswordWithNonce(string password)
        {
            AppConfig.HelperClasses.clsPrivateKeys objclsPrivate = new AppConfig.HelperClasses.clsPrivateKeys();
            ...
            try
            {
                ...
                string nonce = GetSHA1String(createdStr);

                byte[] _nonce = new byte[16];

                _nonce = Convert.FromBase64String(nonce);

                byte[] time = Encoding.UTF8.GetBytes(createdStr);
                ...
                string strpwd = Convert.ToBase64String(sha1.ComputeHash(operand));
                retList.Add(nonce);
                retList.Add(createdStr);
                retList.Add(strpwd);
