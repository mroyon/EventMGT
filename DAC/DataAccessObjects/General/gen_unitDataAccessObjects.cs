...
namespace DAC.Core.DataAccessObjects.General
{
    ...
    internal sealed partial class gen_unitDataAccessObjects : BaseDataAccess, Igen_unitDataAccessObjects
    {
        ...
        #region Logo
        async Task<gen_unitEntity> Igen_unitDataAccessObjects.GetUnitByUserId(string userId, CancellationToken cancellationToken)
        {
            try
            {
                const string SP = "gen_getunitby_userid";
                using (DbCommand cmd = Database.GetStoredProcCommand(SP))
                {
                    Database.AddInParameter(cmd, "@UserId", DbType.String, $"{userId}");
                    var output = new gen_unitEntity();
                    IAsyncResult result = Database.BeginExecuteReader(cmd, null, null);
                    while (!result.IsCompleted)
                    {
                    ...
                    using (IDataReader reader = Database.EndExecuteReader(result))
                    {
                        while (reader.Read())
                        {
                            output = new gen_unitEntity(reader);
                        }
                        reader.Close();
                    }
            ...
            {
                throw GetDataAccessException(ex, SourceOfException("Igen_unitDataAccess.GetUnitByUserId"));
            }
        }
        #endregion
    }
